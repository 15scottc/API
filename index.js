...
app.post('/editStudentByFname', async (req,res)=>{
    try{
        let students = await Student.updateOne({fname : req.body.queryFname}
            , {
                "fname": req.body.fname,
                "lname": req.body.lname
            }, {upsert: true});
...
app.post('/removeStudentFromClasses', async (rec,res)=> {
    try {
        let student = await Student.findOne({studentID: rec.body.studentID});

        if(student){
            await Student.deleteOne({studentID: rec.body.studentID});
